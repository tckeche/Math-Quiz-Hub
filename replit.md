...
- `client/src/pages/StudentDashboard.tsx` - Student dashboard: available quizzes (soma-only via auth-gated `/api/quizzes/available`), completed items (soma reports only), subject donut charts, AI analysis. All quiz links route to `/soma/quiz/:id`, reviews to `/soma/review/:id`. No legacy `/api/quizzes` fetch.
